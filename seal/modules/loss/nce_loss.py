...
class NCERankingLoss(NCELoss):
    def __init__(self, 
                ...
                # {"use_scaling": T/F, "params": [numer, denom1, denom2] } 
                loss_scaling: Dict[str, Any] = {"use_scaling": False}, 
                **kwargs: Any) -> None:
        super().__init__(**kwargs)
    ...
    def compute_loss(
        ...
        if self.use_scorenn:
            ...
            score = 0 
        # y_ = torch.cat((labels.to(dtype=samples.dtype), torch.cat((y_hat, samples), dim=1)), dim=1)
        # buffer['score_dist'] = self.score_nn(x, y_, buffer) - self.distance(y_, y_hat.expand_as(y_))
